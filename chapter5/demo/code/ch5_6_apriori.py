...
def find_rule(d, support, confidence, ms=u"--"):
    ...
    while len(column) > 1:
        ...
        def sf(i):
            return d[i].prod(axis=1, numeric_only=True)  # 新一批支持度的计算函数

        # 创建连接数据，这一步耗时、耗内存最严重。当数据集较大时，可以考虑并行运算优化。
        d_2 = pd.DataFrame(
            list(map(sf, column)), index=[ms.join(i) for i in column]
        ).T

        support_series_2 = (
            1.0 * d_2[[ms.join(i) for i in column]].sum() / len(d)
        )  # 计算连接后的支持度
        column = list(
        ...
        for i in column2:  # 计算置信度序列
            cofidence_series[ms.join(i)] = (
                support_series[ms.join(sorted(i))]
                / support_series[ms.join(i[: len(i) - 1])]
            )

        for i in cofidence_series[
            cofidence_series > confidence
        ].index:  # 置信度筛选
            result[i] = 0.0
